...
export class PublicationSyncJobService {
  private readonly logger = new Logger(
    PublicationSyncJobService.name,
  );
  ...
  async processNext(
    ...
    const existing = await this.findOwnedJob(
      syncId,
    ...
    if (existing.status === 'FAILED') {
      throw new ConflictException(
        'La sincronización finalizó con error',
      );
    }
    ...
    try {
      ...
      return await this.processClaimedJob(
        job,
        access,
      );
    } catch (error) {
      ...
      if (isRetryableError(error)) {
        await this.jobsRepository.releaseAfterError(
          job.id,
          safeFatalMessage(error),
        );

        throw error;
      }

      await this.jobsRepository.fail(
        job.id,
        safeFatalMessage(error),
      );
  ...
  async getStatus(
    ...
    const job = await this.findOwnedJob(
      syncId,
      connection.seller_id,
    );
  ...
  private async processClaimedJob(
    ...
    const scan = await this.ensureBuffer(
      job,
      access,
    );
    ...
    const batchIds = scan.bufferItemIds.slice(
      0,
      PUBLICATION_SYNC_BATCH_SIZE,
    );
    ...
    const updated =
      await this.jobsRepository.updateProgress(
        job.id,
        {
  ...
  private async ensureBuffer(
    ...
    const page =
      await this.sourceService.fetchNextScanPage(
        access.sellerId,
        access.accessToken,
        job.scan_started
          ? job.scroll_id ?? undefined
          : undefined,
      );
  ...
  private async finishJob(
    ...
    try {
      completed =
        await this.jobsRepository.complete(
          job.id,
        );
    } catch (error) {
  ...
  private async findOwnedJob(
    ...
    const job =
      await this.jobsRepository.findById(
        syncId,
      );
  ...
  /** Registra el error real solamente en los logs del backend. */
  private logSyncError(
    ...
    this.logger.error(
      `Falló sincronización ${syncId}: ${message}`,
    );
  }
}
...
class CompletionPersistenceError extends Error {
  constructor(
    ...
    super(
      'No se pudo persistir la finalización',
    );
  }
}
