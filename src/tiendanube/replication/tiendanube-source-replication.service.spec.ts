...
const SHARED_OWNER_USER_ID = 'prueba';
...
describe('TiendanubeSourceReplicationService', () => {
  ...
  it('usa el owner de la conexión compartida al reservar y completar', async () => {
    connection.findOwnedCredentialsByUserId.mockResolvedValue({
      userId: SHARED_OWNER_USER_ID,
      storeId: STORE_ID,
      accessToken: 'tn-token',
      scope: 'write_products',
    });

    await service.replicate(AUTHENTICATED_USER_ID, SOURCE_KEY);
  ...
  it('usa el owner de la conexión compartida al marcar un fallo', async () => {
    ...
    await expect(
      service.replicate(AUTHENTICATED_USER_ID, SOURCE_KEY),
